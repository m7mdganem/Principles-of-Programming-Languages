...
export const extendStore = (s: Store, val: Value): Store => {
    ...
    // Complete
export const addValueToStoreAndGetAddress = (s: Store, val: Value): number => extendStore(s,val).vals.length - 1;
    
export const applyStore = (store: Store, address: number): Result<Value> => 
    (address >= 0 && address < store.vals.length) ? makeOk(unbox(store.vals[address])) :
    makeFailure("Address is not in the heap");
    // Complete
...
const applyGlobalEnv = (env: GlobalEnv, v: string): Result<number> => {
    const index = unbox(env.vars).indexOf(v);
    return (index > -1) ? makeOk(unbox(env.addresses)[index]) : makeFailure(`Var not found: ${v}`);
};
    // Complete
...
export const applyEnvStore = (env: Env, store: Store, v: string) : Result<Value> =>
    bind(applyEnv(env,v),
            (address: number) => applyStore(store, address));
