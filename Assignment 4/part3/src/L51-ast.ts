...
const parseGoodClassExp = (typeName: Sexp, varDecls: Sexp, bindings: Sexp): Result<ClassExp> =>
    ! isArray(varDecls) ? makeFailure(`Invalid args ${JSON.stringify(varDecls)}`):
    ! isGoodBindings(bindings) ? makeFailure(`Invalid bindings: ${JSON.stringify(bindings)}`):
    ! isString(typeName) ? makeFailure(`Invalid type name ${JSON.stringify(typeName)}`):
    parseClass(typeName, varDecls, bindings);


const parseClass = (typeName: string, varDecls: Sexp[], bindings: [Sexp, Sexp][]): Result<ClassExp> => {
    const fields: Result<VarDecl[]> = mapResult(parseVarDecl,varDecls);
    const methods: Result<Binding[]> = parseBindings(bindings);
    const tName: Result<TVar> = makeOk(makeTVar(typeName));
    return safe3((tName: TVar, fields: VarDecl[], methods: Binding[]) => makeOk(makeClassExp(tName,fields,methods)))(tName, fields, methods);
}
...
export const parsedToClassExps = (p: Parsed): ClassExp[] => 
    isProgram(p) && !isEmpty(p.exps) ? parseExpsToClass(p.exps):
    isExp(p) ? parseExpsToClass([p]):
    [];

const parseExpsToClass = (exps: Exp[]): ClassExp[] => 
    isEmpty(exps) ? [] :
    isDefineExp(exps[0]) ?  parseCExpsToClass(exps[0].val).concat(parseExpsToClass(rest(exps))):
    isCExp(exps[0]) ? parseCExpsToClass(exps[0]).concat(parseExpsToClass(rest(exps))):
    [];

const parseCExpsToClass = (exp: CExp): ClassExp[] =>
     isAppExp(exp) ? parseCExpsToClass(exp.rator).concat(parseExpsToClass(exp.rands)):
     isIfExp(exp) ? parseCExpsToClass(exp.test).concat(parseCExpsToClass(exp.then)).concat(parseCExpsToClass(exp.alt)):
     isProcExp(exp) ? parseExpsToClass(exp.body):
     isLetExp(exp) ? parseExpsToClass(map((B:Binding) => B.val, exp.bindings)).concat(parseExpsToClass(exp.body)):
     isLetrecExp(exp) ? parseExpsToClass(map((B:Binding) => B.val, exp.bindings)).concat(parseExpsToClass(exp.body)):
     isSetExp(exp) ? parseCExpsToClass(exp.val):
     isClassExp(exp) ? [exp].concat(parseExpsToClass(map((B:Binding) => B.val, exp.methods))):
     [];
