   ...
   (cond ((empty? tree1)
         ...
         (else
          (equal-trees$ (car tree1) (car tree2)
                        (lambda (first-res) (equal-trees$ (cdr tree1) (cdr tree2)
                                                          (lambda (rest-res) (succ (cons first-res rest-res)))
                                                          fail))
                        fail)))
 )
)
...
(define reduce3-lzl 
  (lambda (reducer init lzl)
    (if (empty? lzl)
        empty-lzl
        (cons (reducer init (car lzl)) (lambda () (reduce3-lzl reducer (reducer init (car lzl)) ((cdr lzl))))))
  )
)  
...
(define generate-pi-approximations
  (lambda ()
    (reduce3-lzl + 0 (map-lzl (lambda (x) (* 8 (/ 1 (* x (+ x 2))))) (integers-steps-from 1 4)))
  )
)