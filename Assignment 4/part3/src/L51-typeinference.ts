...
export const makeTEnvFromClasses = (parsed: A.Parsed): E.TEnv => {
    ...
    const typeNames: string[] = R.map((cl: A.ClassExp) => cl.typeName.var,allClasses);
    const TExps: T.ClassTExp[] = R.map((cl: A.ClassExp) => A.classExpToClassTExp(cl), allClasses)
    return E.makeExtendTEnv(typeNames,TExps,E.makeEmptyTEnv());
}
...
export const typeofDefine = (exp: A.DefineExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    ...
    const valTE = typeofExp(exp.val,E.makeExtendTEnv([exp.var.var],[varTE],tenv));
    const constraint = bind(valTE, (valTE: T.TExp) => checkEqualType(varTE,valTE,exp));
...
const typeofProgramExps = (exp: A.Exp, exps: A.Exp[], tenv: E.TEnv): Result<T.TExp> =>
    ...
    A.isDefineExp(exp) ? bind(typeofDefine(exp, tenv), _ => typeofProgram(A.makeProgram(exps), E.makeExtendTEnv([exp.var.var],[exp.var.texp],tenv))) :
    bind(typeofExp(exp,tenv), _ => typeofProgram(A.makeProgram(exps), tenv));
...
export const typeofLit = (exp: A.LitExp): Result<T.TExp> =>
    ...
    makeFailure(`LitExp should be Symbol or Pair: ${JSON.stringify(exp.val)}`);
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
    ...
    return bind(safe2((varTE: T.TExp, valTE: T.TExp) => checkEqualType(varTE,valTE,exp))(varTE,valTE), _ => makeOk(T.makeVoidTExp()));
};
...
export const typeofClass = (exp: A.ClassExp, tenv: E.TEnv): Result<T.TExp> => {
    const fieldsTEs = R.map((vardecl: A.VarDecl) => vardecl.texp, exp.fields);
    const extTEnv = E.makeExtendTEnv(R.map((vardecl: A.VarDecl) => vardecl.var, exp.fields), fieldsTEs, tenv);
    const TypesofMethods: Result<T.TExp[]> = mapResult((b: A.Binding)=>typeofExp(b.val,extTEnv),exp.methods);
    const methodsTEs = R.map((b: A.Binding) => b.var.texp ,exp.methods);
    const constraint = bind(TypesofMethods, (TExp: T.TExp[]) => checkEqualTypes(TExp,methodsTEs,exp));
    return bind(constraint, _ => makeOk(T.makeProcTExp(fieldsTEs, A.classExpToClassTExp(exp))));
};