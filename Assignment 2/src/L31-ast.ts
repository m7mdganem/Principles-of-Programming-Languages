...
// CompoundCExp -> IfExp | ProcExp | ClassExp | LetExp | LitExp | AppExp 
...
const parseClassExp = (vardecls: Sexp, bindings: Sexp): Result<ClassExp> => {
    ...
    const varDeclsResult = mapResult((x:string) => makeOk(makeVarDecl(x)), vardecls);
    const vars = map(b => b[0], bindings);
    const valsResult = mapResult(binding => parseL31CExp(first(rest(binding))), bindings);
    const bindingsResult = bind(valsResult, (vals: CExp[]) => makeOk(zipWith(makeBinding, vars, vals)));
    
    return safe2((vardecls: VarDecl[], bindings: Binding[]) => makeOk(makeClassExp(vardecls, bindings)))
        (varDeclsResult,bindingsResult);        
}
